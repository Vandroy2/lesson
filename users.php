...
if (isset($_GET['id'])){
    ...
    echo "Сортировка по surname";
    usort($users, function($user1, $user2){
        return strcmp($user1->surname, $user2->surname);
    });}

    if (isset($_GET['email'])){
    echo "Сортировка по email";
    usort($users, function($user1, $user2){
...
<tr>
    ...
    foreach($users as $user){if($userId == $user->id){
      ...
      echo "Удаление инициализированно";
    ...
    // $array = array_diff($users, [$userAr]);

    // $array = array_diff_key($users, [0 => $userAr]);

    // $array = array_diff_key($users, [0 => $user]);
    ...
    if (isset($_GET['ide'])){
    $userId = intval($_GET['ide']);
    foreach($users as $user){if($userId == $user->id){
        echo "Начата подготовка к редактированию";
